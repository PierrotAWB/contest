...
void generatePairings(set<int> s, vector<pair<int, int> > v, int index)
{
	...
	for (set<int>::iterator it = s.begin(); it != s.end();)
	{
		...
		// there is neither undefined behaviour 
...
bool hasLoop(vector<pair<int, int> > pairing) {
	for (int i = 0; i < 2 * pairing.size(); i++) {
		// each of these 2 (N/2) wormholes could be the start of a loop
		int index = 0; // index for a particular visited array
		vector<int> visited; 
